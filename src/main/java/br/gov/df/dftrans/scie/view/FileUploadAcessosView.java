...
public class FileUploadAcessosView {
	...
	public void setUpload() {
		...
		File file = new File(current + "" + delimitadorDiretorio 
				...
				+ getCpf() + "" + delimitadorDiretorio + "" + fmt.format(date) 
				+ "" + delimitadorDiretorio + "" + DOC1
		...
		if (file.exists()) {
			...
			iconeDOC1 = "//resources//images//" + (esconderArquivos[0] ? 
					"dis" : "un") + "checked-icon.png";
		}
		...
		if (file.exists()) {
			...
			isUploaded.add(new Boolean(false));
			iconeDOC2 = "//resources//images//" + (esconderArquivos[1] ? 
					"dis" : "un") + "checked-icon.png";
		}
	...
	/**
	 * Método que trata o upload de arquivos de acordo com um parâmetro de documento;
	...
	public void doUpload(FileUploadEvent fileUploadEvent, int documento) {
		...
		if (getCpf() == null) {
			try {
				...
				.redirect("estudanteAcessosHome.xhtml");
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
		...
		files[documento] = current + "" + delimitadorDiretorio 
				...
				+ "" + delimitadorDiretorio + ""
				+ documento + "" + delimitadorDiretorio + "" 
				+ nomesArquivos[documento] + "." + aux[aux.length - 1];
		conteudo = new ArrayList<String>();
	...
	public void copiarArquivoPDF(String path, UploadedFile uploadedFile, int documento) {
		try {
			...
			FileOutputStream os = new FileOutputStream(current + path 
					+ "." + aux[aux.length - 1]);
	...
	public void atualizarCampos() {
		...
		case (2):
			...
			break;
		case (3):
			descricaoArquivos[2] = "Neste caso serão confrontados os arquivos "
					+ "já anexados do comprovante de residência do estudante "
					+ "e o endereço de sua Instituição de Ensino no cadastro "
					+ "já realizado. NÃO HÁ NECESSIDADE DE ENVIO DE ARQUIVO!!!";
			descricaoArquivos[3] = "";
			break;
		case (4):
	...
	/**
	 * Método que seta as estruturas e variáveis do bean ConcatenaArquivos
	 * e retorna o redirecionamento à pagina de concatenação de arquivos.
	 * @return
	 */
	public String concatenaArquivos() {
		FacesContext context = FacesContext.getCurrentInstance();
		ELResolver resolver = context.getApplication().getELResolver();
		ConcatenaArquivo bean = (ConcatenaArquivo) resolver.getValue(context
				.getELContext(), null, "fileUploadConcatenaView");
		bean.setOrigem(2);
